 ...
 */
static int llgclist_put_head(uint32_t *list, uint32_t size, uint64_t *head, uint64_t *tail, uint32_t *free, uint32_t value);
static int llgclist_put_tail(uint32_t *list, uint32_t size, uint64_t *head, uint64_t *tail, uint32_t *free, uint32_t value);

/**
 * Pop a value from the list, and return 0.
 * If this returns 0, there was no value to pop.
 */
static int llgclist_get_head(uint32_t *list, uint32_t size, uint64_t *head, uint64_t *tail, uint32_t *free, uint32_t *value);
static int llgclist_get_tail(uint32_t *list, uint32_t size, uint64_t *head, uint64_t *tail, uint32_t *free, uint32_t *value);
...
#define GCLIST_LINE_MASK  (-(LINE_SIZE))
#define GCLIST_CNT_MASK   ((LINE_SIZE)-1)
#define ADD_TO_GET_LAST   (((LINE_SIZE)/sizeof(uint32_t))-1)
#define GCLIST_HEAD(a)    (((uint32_t)(a >> 36)) & 0x0FFFFFFF)
#define GCLIST_TAIL(a)    (((uint32_t)(a >> 8))  & 0x0FFFFFFF)
#define GCLIST_CNTR(a)    (((uint32_t)(a))       & 0x000000FF)
#define GCLIST_MK(h,t,c)  ( (((uint64_t)h)<<36) | (((uint64_t)t)<<8) | (c&0x000000FF) )

/**
 * This is a LL ring buffer for 31-bit values with loose ordening.
 * The trick is to reserve the highest bit for state: either EMPTY or FILLED.
 * HEAD_TAIL is 64 bytes... 28+28=56 and 8 for counter
 */
 
static int llgclist_init(uint32_t *list, uint32_t size, uint64_t *head_tail, uint32_t *free)
{
    *head_tail = 0;
    *free = size;
}

static int llgclist_put_head(uint32_t *list, uint32_t size, uint64_t *head_tail, uint32_t value)
{
    value |= 0x80000000;
    ...
    // Find slot
    uint64_t ht = *(volatile uint64_t*)head_tail;
    uint32_t slot = GCLIST_HEAD(ht);
    while (1) {
        uint32_t v = *(volatile uint32_t*)&list[slot];
        if (!(v & 0x8000000)) {
            if (cas(&list[slot], v, value)) {
                // success, update head
                while (1) {
                    ht = *(volatile uint64_t*)head_tail;
                    uint32_t h2 = GCLIST_HEAD(ht);
                    uint32_t t2 = GCLIST_TAIL(ht);
                    uint8_t cnt = GCLIST_CNTR(ht) + 1;
                    /* Greedy update.
                     * we update if the head has not been moved to the left.
                     * two cases:     [ ... H2 ... SLOT ... TAIL ... ] = H2 < SLOT
                     *                [ ... TAIL ... H2 ... SLOT ... ] = H2 < SLOT
                     *                [ ... SLOT ... TAIL ... H2 ... ] = SLOT < TAIL && TAIL < H2
                     */
                    if (h2 < slot || (slot < t2 && t2 < h2)) break;
                    uint64_t ht_new = GCLIST_MK(slot, t2, cnt);
                    if (cas(head_tail, ht, ht_new)) break;
                }
                break;
            }
        }
        ...
        treshold--;
        if (treshold == 0) return 0;
    }
    return 1;
}

static int llgclist_put_tail(uint32_t *list, uint32_t size, uint32_t *head, uint32_t *tail, uint32_t *free, uint32_t value)
{
    // Reduce free
    while (1) {
        uint32_t f = *(volatile uint32_t*)free;
        if (f <= 0) return 0;
        if (cas(free, f, f-1)) break;
    }
    
    value |= 0x80000000;
    
    // Find slot
    uint32_t slot = *(volatile uint32_t*)tail;
    while (1) {
        uint32_t v = *(volatile uint32_t*)&list[slot];
        if (!(v & 0x8000000)) {
            if (cas(&list[slot], v, value)) {
 ...
 * If this returns 0, there was no value to pop.
 */
static int llgclist_get_head(llgcset_gclist_t *list, uint32_t size, uint32_t *head, uint32_t *tail, uint32_t *value);
static int llgclist_get_tail(llgcset_gclist_t *list, uint32_t size, uint32_t *head, uint32_t *tail, uint32_t *value);
