...
void *stmemo_lookup_hash(const stmemo_t dbs, const void* data, int* created, uint32_t* index, uint32_t* hash)
{
    size_t              seed = 0;
    size_t              l = dbs->length;
    size_t              b = dbs->bytes;
...
/*
inline void *stmemo_index_to_ptr(const stmemo_t dbs, uint32_t index)
{
...
*/
inline void *stmemo_get_or_create(const stmemo_t dbs, const void *data, int *created, uint32_t *index)
{
	int _created;
	uint32_t _index;
    void *result = stmemo_lookup_hash(dbs, data, &_created, &_index, NULL);
...
{
    stmemo_t dbs = rt_align(CACHE_LINE_SIZE, sizeof(struct stmemo));
    dbs->length = length;
...
inline void stmemo_clear(stmemo_t dbs)
{
    memset(dbs->table, 0, sizeof(uint32_t) * dbs->size);
}
