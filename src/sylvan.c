...
static BDD sylvan_makeite(int thread, BDD a, BDD b, BDD c, int *created, int *cached)
{
    ...
    if (BDD_STRIPMARK(a) == BDD_STRIPMARK(b)) {
        // we can ignore constants, so use faster STRIPMARK
 ...
 * Marks on the cache_low and cache_high will be transferred.
 */
static inline void sylvan_move_parents(bddcache_t from, BDD to_c)
{
    ...
    bddcache_t parent;
    while (llvector_count(&from->parents) > 0) {
        if (!llvector_pop(&from->parents, &parent)) assert(0);

        if (BDD_STRIPMARK(parent->cache_low) == from_c) {
            parent->cache_low = BDD_TRANSFERMARK(parent->cache_low, to_c);
        }

        if (BDD_STRIPMARK(parent->cache_high) == from_c) {
            parent->cache_high = BDD_TRANSFERMARK(parent->cache_high, to_c);
        }

        llvector_add(&to->parents, &parent);
    }
...
static inline int sylvan_process_ite_ex(int thread, bddcache_t node, int queue_new_nodes)
{
    ...
    if (is_not_root_node == 0) {
        ...
        llvector_add(&GETCACHE(result)->parents, &node);
    } else {
    ...
    if (created) {
        if (queue_new_nodes==0) {
            llvector_add(&_bdd.leaves2, &result);
        } else {
...
static inline void sylvan_handle_ite_parents(const int thread, bddcache_t node, BDD node_c)
{
    ...
    while (llvector_pop(&node->parents, &parent)) {
        ...
        if (parent->cache_low == parent->cache_high) {
            // For example, the ITE* root node has this property
            ...
            if (BDD_STRIPMARK(parent->cache_low) == node_c) {
                atomicsylvan_write(&parent->low, BDD_TRANSFERMARK(parent->cache_low, q));
                parent->cache_low = 0;
            }

            if (BDD_STRIPMARK(parent->cache_high) == node_c) {
                atomicsylvan_write(&parent->high, BDD_TRANSFERMARK(parent->cache_high, q));
                parent->cache_high = 0;
            }

            // Memory barrier
            // it turns out lfence, sfence, mfence are all about as fast for now
            __sync_synchronize();

            // Use volatiles!
            if (atomicsylvan_read(&parent->low) == sylvan_invalid ||
                atomicsylvan_read(&parent->high) == sylvan_invalid) continue;

            if (!cas(&parent->result, sylvan_invalid, bddhandled)) continue;
        }

        // We're here, the parent has both dependencies resolved
        if ((parent->a & bddinternal) == 0) {
            // The parent is a normal ITE node
            if (parent->low == parent->high) {
                parent->result = q; // this is for the cache

                // Take over parents and continue
                sylvan_move_parents(parent, node_c);
                continue;
            }

            parent->result = sylvan_makenode(parent->root, parent->low, parent->high);
            llsched_push(_bdd.sched, thread, &parent_c);
            continue;
        }

        if (sylvan_process_ite_ex(thread, parent, 1)) {
            llsched_push(_bdd.sched, thread, &parent_c);
        }
    }
...
static inline void sylvan_prepare_ite(const int thread, bddcache_t node, BDD node_c)
{
    ...
    // Step 1: figure out what our head level is
    ...
    bddcache_t low_node=0, high_node=0;
    ...
    if (cached) {
        ...
        low_node = GETCACHE(low);

        node->cache_low = low;
        llvector_add(&low_node->parents, &node);
    }
    ...
    if (cached) {
        ...
        high_node = GETCACHE(high);

        node->cache_high = high;
        llvector_add(&high_node->parents, &node);
    }

    // By this time, strange things might have happened!
    // if (n!=2) __sync_synchronize();

    // Perhaps the result just got set?
    if (low_node && atomicsylvan_read(&low_node->result) != sylvan_invalid) {
        while (atomicsylvan_read(&low_node->result) == bddhandled) {}
        sylvan_handle_ite_parents(thread, low_node, BDD_STRIPMARK(low));
    }

    if (high_node && atomicsylvan_read(&high_node->result) != sylvan_invalid) {
        while (atomicsylvan_read(&high_node->result) == bddhandled) {}
        sylvan_handle_ite_parents(thread, high_node, BDD_STRIPMARK(high));
    }

    if (n==2) {
        // low and high are cached, so this is a leaf in the operation
        // calculate result and add to job queue
        if (node->low == node->high) node->result = node->low;
        else node->result = sylvan_makenode(node->root, node->low, node->high);
...
/*
static void sylvan_execute_ite(const int thread) {
    // Phase 1: create dependency tree and leafs
    BDD node_c;
    while (llsched_pop(_bdd.sched, thread, &node_c) == 1) {
        node_c = BDD_STRIPMARK(node_c);
        ...
        if (cached) {
            ...
            node->low = sylvan_invalid;
            node->cache_low = low;
            llvector_add(&GETCACHE(low)->parents, &node);
        }
        ...
        if (cached) {
            ...
            node->high = sylvan_invalid;
            node->cache_high = high;
            llvector_add(&GETCACHE(high)->parents, &node);
        }
        ...
        if (n==2) {
            ...
            llvector_add(&_bdd.leaves, &node_c);
        }
    }
    ...
    if (thread == 0) {
        ...
        while (llvector_count(&_bdd.leaves)>0) {
            if (!llvector_pop(&_bdd.leaves, &node_c)) assert(0);
    ...
    while (llsched_pop(_bdd.sched, thread, &node_c) == 1) {
        ...
        assert (q!=sylvan_invalid &&  q!=bddhandled);

        while (llvector_count(&node->parents) > 0) {
            bddcache_t parent;
            if (!llvector_pop(&node->parents, &parent)) assert(0);
            ...
            if (parent->cache_low == parent->cache_high) {
                ...
                if (parent->low == sylvan_invalid || parent->high == sylvan_invalid) continue;
                if (!cas(&parent->result, sylvan_invalid, bddhandled)) continue;
            }
...
static void sylvan_execute_ite_down(const int thread) {
    // Phase 1: create calculation tree
    BDD node_c;
    while (llsched_pop(_bdd.sched, thread, &node_c) == 1) {
        node_c = BDD_STRIPMARK(node_c);
        bddcache_t node = GETCACHE(node_c);
        ...
        assert(!a || a->low != bddhandled);
        assert(!b || b->low != bddhandled);
        assert(!c || c->low != bddhandled);
        assert(!a || a->high != bddhandled);
        assert(!b || b->high != bddhandled);
        assert(!c || c->high != bddhandled);
        ...
        // Determine if we need child ITE* node
        if (level < _bdd.replaceLast) {
            ...
            int created;
            bddcache_t ptr = llset_get_or_create(_bdd.cache, template_apply_node[thread], &created, &idx);
            if (created) llsched_push(_bdd.sched, thread, &idx);

            node->cache_low = idx;
            node->low = sylvan_invalid;
            llvector_add(&ptr->parents, &node);
        } else {
            // we need a child ITE node (no ITE*)
            int created, cached;
            BDD result = sylvan_makeite(thread, aLow, bLow, cLow, &created, &cached);
            if (cached) {
                node->cache_low = 0;
                node->low = result;
                ++n;
            } else {
                if (created) llvector_add(&_bdd.leaves2, &result);
        ...
        if (level < _bdd.replaceLast) {
            ...
            // we need a child ITE node (no ITE*)
            int created, cached;
            BDD result = sylvan_makeite(thread, aHigh, bHigh, cHigh, &created, &cached);
            if (cached) {
                node->cache_high = 0;
                node->high = result;
                ++n;
    ...
    while (llsched_pop(_bdd.sched, thread, &node_c) == 1) {
        ...
        if (!sylvan_process_ite_ex(thread, node, 0)) continue;
        ...
        bddcache_t parent;
        while (llvector_count(&node->parents) > 0) {
            llvector_pop(&node->parents, &parent);
            BDD parent_c = GETCACHEBDD(parent);
            ...
            if (parent->low != sylvan_invalid && parent->high != sylvan_invalid) {
                if (cas(&parent->result, sylvan_invalid, bddhandled)) {
                    if (sylvan_process_ite_ex(thread, parent, 0)) {
...
void sylvan_print(BDD bdd) {
    ...
    while (llvector_count(v) > 0) {
        ...
        if (low >= 2) {
            ...
            if (created) llvector_add(v, &low);
        }
        if (high >= 2) {
            llset_get_or_create(s, &high, &created, NULL);
            if (created) llvector_add(v, &high);
        }
    }
