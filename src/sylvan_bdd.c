...
TASK_IMPL_4(BDD, sylvan_ite, BDD, a, BDD, b, BDD, c, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_ITE), b, c, result);
    }
...
TASK_IMPL_3(BDD, sylvan_constrain, BDD, a, BDD, b, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_CONSTRAIN), b, 0, result);
    }
...
TASK_IMPL_3(BDD, sylvan_restrict, BDD, a, BDD, b, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_RESTRICT), b, 0, result);
    }
...
TASK_IMPL_3(BDD, sylvan_exists, BDD, a, BDD, variables, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_EXISTS), variables, 0, result);
    }
...
TASK_IMPL_4(BDD, sylvan_and_exists, BDD, a, BDD, b, BDDSET, v, BDDVAR, prev_level)
{
    ...
    if (sylvan_set_isempty(v)) return CALL(sylvan_ite, a, b, sylvan_false, 0);
    // v cannot be sylvan_false
    ...
    while (nv->level < level) {
        v = node_high(v, nv);
        if (v == sylvan_true) return CALL(sylvan_ite, a, b, sylvan_false, 0);
        nv = GETNODE(v);
    }
    ...
    if (nv->level == level) {
        // level is in variable set, perform abstraction
        BDD low = CALL(sylvan_and_exists, aLow, bLow, node_high(v, nv), level);
        if (low == sylvan_true) {
            result = sylvan_true;
        } else {
            bdd_refs_push(low);
            BDD high = CALL(sylvan_and_exists, aHigh, bHigh, node_high(v, nv), level);
            if (high == sylvan_true) {
                result = sylvan_true;
            }
            else if (low == sylvan_false && high == sylvan_false) {
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_AND_EXISTS), b, v, result);
    }
...
TASK_IMPL_4(BDD, sylvan_relnext, BDD, a, BDD, b, BDDSET, vars, BDDVAR, prev_level)
{
    ...
    if (vars == sylvan_false || level == vars_node->level || (level^1) == vars_node->level) {
        /* Get s and t */
        BDDVAR s = level & (~1);
        BDDVAR t = s+1;
        ...
        BDD _vars = vars == sylvan_false ? sylvan_false : node_high(vars, vars_node);
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_RELNEXT), b, vars, result);
    }
...
TASK_IMPL_4(BDD, sylvan_relprev, BDD, a, BDD, b, BDDSET, vars, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_RELPREV), b, vars, result);
    }
...
TASK_IMPL_2(BDD, sylvan_closure, BDD, a, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_CLOSURE), 0, 0, result);
    }
...
TASK_IMPL_3(BDD, sylvan_compose, BDD, a, BDDMAP, map, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        cache_put(BDD_SETDATA(a, CACHE_COMPOSE), map, 0, result);
    }
...
TASK_IMPL_3(double, sylvan_satcount, BDD, bdd, BDDSET, variables, BDDVAR, prev_level)
{
    ...
    if (cachenow) {
        hack.d = result;
        cache_put(BDD_SETDATA(bdd, CACHE_SATCOUNT), variables, 0, hack.s);
    }
