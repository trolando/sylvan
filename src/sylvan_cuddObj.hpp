...
#ifndef SYLVAN_CUDDOBJ_HPP
#define SYLVAN_CUDDOBJ_HPP
...
protected:
    ABDD();
    ABDD(Cudd const & manager, DdNodePtr ddNode);
    ABDD(const ABDD &from);
    ABDD(DdNodePtr ddNode);
    ~ABDD();
public:
...
public:
    BDD();
    BDD(Cudd const & manager, DdNodePtr ddNode);
    BDD(DdNodePtr ddNode);
    BDD(const BDD &from);
...
public:
    ADD();
    ADD(Cudd const & manager, DdNodePtr ddNode);
    ADD(DdNodePtr ddNode);
    ADD(const ADD &from);
...
class Cudd {
    ...
    friend std::ostream & operator<<(std::ostream & os, BDD const & f);
public:
    Cudd(unsigned int numVars = 0, unsigned int numVarsZ = 0, unsigned int numSlots = 0, unsigned int cacheSize = 0, unsigned long maxMemory = 0, PFC defaultHandler = NULL);
    ~Cudd(void);
    ...
    /**
     * Return MTBDD representing 1.0
     */
    ADD addOne(void) const;
    /**
     * Return MTBDD representing 0.0
     */
