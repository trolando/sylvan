...
{
    const TBDD **pbegin, **pend, **pcur;
    TBDD *rbegin, *rend, *rcur;
    Task **sbegin, **send, **scur;
} *tbdd_refs_internal_t;

DECLARE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);

VOID_TASK_2(tbdd_refs_mark_p_par, const TBDD**, begin, size_t, count)
{
    if (count < 32) {
        while (count) {
            tbdd_gc_mark_rec(**(begin++));
            count--;
        }
    } else {
        SPAWN(tbdd_refs_mark_p_par, begin, count / 2);
        CALL(tbdd_refs_mark_p_par, begin + (count / 2), count - count / 2);
        SYNC(tbdd_refs_mark_p_par);
    }
}

VOID_TASK_2(tbdd_refs_mark_r_par, TBDD*, begin, size_t, count)
{
    if (count < 32) {
        while (count) {
            tbdd_gc_mark_rec(*begin++);
        ...
        SPAWN(tbdd_refs_mark_r_par, begin, count / 2);
        CALL(tbdd_refs_mark_r_par, begin + (count / 2), count - count / 2);
        SYNC(tbdd_refs_mark_r_par);
    }
}

VOID_TASK_2(tbdd_refs_mark_s_par, Task**, begin, size_t, count)
{
    if (count < 32) {
        while (count) {
            Task *t = *begin++;
            ...
            if (TASK_IS_COMPLETED(t)) {
                tbdd_gc_mark_rec(*(TBDD*)TASK_RESULT(t));
            }
            count--;
        }
    } else {
        if (!TASK_IS_STOLEN((*begin))) return;
        SPAWN(tbdd_refs_mark_s_par, begin, count / 2);
        CALL(tbdd_refs_mark_s_par, begin + (count / 2), count - count / 2);
        SYNC(tbdd_refs_mark_s_par);
    }
}
...
VOID_TASK_0(tbdd_refs_mark_task)
{
    ...
    SPAWN(tbdd_refs_mark_p_par, tbdd_refs_key->pbegin, tbdd_refs_key->pcur-tbdd_refs_key->pbegin);
    SPAWN(tbdd_refs_mark_r_par, tbdd_refs_key->rbegin, tbdd_refs_key->rcur-tbdd_refs_key->rbegin);
    CALL(tbdd_refs_mark_s_par, tbdd_refs_key->sbegin, tbdd_refs_key->scur-tbdd_refs_key->sbegin);
    SYNC(tbdd_refs_mark_r_par);
    SYNC(tbdd_refs_mark_p_par);
}
...
VOID_TASK_0(tbdd_refs_init_task)
{
    ...
    s->pcur = s->pbegin = (const TBDD**)malloc(sizeof(TBDD*) * 1024);
    s->pend = s->pbegin + 1024;
    s->rcur = s->rbegin = (TBDD*)malloc(sizeof(TBDD) * 1024);
...
void
tbdd_refs_ptrs_up(tbdd_refs_internal_t tbdd_refs_key)
{
    size_t cur = tbdd_refs_key->pcur - tbdd_refs_key->pbegin;
    size_t size = tbdd_refs_key->pend - tbdd_refs_key->pbegin;
    tbdd_refs_key->pbegin = (const TBDD**)realloc(tbdd_refs_key->pbegin, sizeof(TBDD*) * size * 2);
    tbdd_refs_key->pcur = tbdd_refs_key->pbegin + cur;
    tbdd_refs_key->pend = tbdd_refs_key->pbegin + (size * 2);
}

TBDD __attribute__((noinline))
tbdd_refs_refs_up(tbdd_refs_internal_t tbdd_refs_key, TBDD res)
{
    long size = tbdd_refs_key->rend - tbdd_refs_key->rbegin;
    tbdd_refs_key->rbegin = (TBDD*)realloc(tbdd_refs_key->rbegin, sizeof(TBDD) * size * 2);
    tbdd_refs_key->rcur = tbdd_refs_key->rbegin + size;
    tbdd_refs_key->rend = tbdd_refs_key->rbegin + (size * 2);
    return res;
}

void __attribute__((noinline))
tbdd_refs_tasks_up(tbdd_refs_internal_t tbdd_refs_key)
{
    long size = tbdd_refs_key->send - tbdd_refs_key->sbegin;
    tbdd_refs_key->sbegin = (Task**)realloc(tbdd_refs_key->sbegin, sizeof(Task*) * size * 2);
    tbdd_refs_key->scur = tbdd_refs_key->sbegin + size;
    tbdd_refs_key->send = tbdd_refs_key->sbegin + (size * 2);
}

void __attribute__((unused))
tbdd_refs_pushptr(const TBDD *ptr)
{
    LOCALIZE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);
    *tbdd_refs_key->pcur++ = ptr;
    if (tbdd_refs_key->pcur == tbdd_refs_key->pend) tbdd_refs_ptrs_up(tbdd_refs_key);
}

void __attribute__((unused))
tbdd_refs_popptr(size_t amount)
{
    LOCALIZE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);
    tbdd_refs_key->pcur -= amount;
}

TBDD __attribute__((unused))
tbdd_refs_push(TBDD tbdd)
{
    LOCALIZE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);
    *(tbdd_refs_key->rcur++) = tbdd;
    if (tbdd_refs_key->rcur == tbdd_refs_key->rend) return tbdd_refs_refs_up(tbdd_refs_key, tbdd);
    else return tbdd;
}

void __attribute__((unused))
tbdd_refs_pop(long amount)
{
    LOCALIZE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);
    tbdd_refs_key->rcur -= amount;
}
...
{
    LOCALIZE_THREAD_LOCAL(tbdd_refs_key, tbdd_refs_internal_t);
    *(tbdd_refs_key->scur++) = t;
    if (tbdd_refs_key->scur == tbdd_refs_key->send) tbdd_refs_tasks_up(tbdd_refs_key);
}
