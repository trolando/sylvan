...
TASK(int, zdd_extend_domain_internal, ZDD*, result, ZDD, dd, ZDD, newvars, int, value)
...
{
    ZDD result;
    if (_zdd_try_make_node(&result, var, low, high) != SYLVAN_OK) {
        fprintf(stderr, "BDD Unique table full, %zu of %zu buckets filled!\n", nodes_count_nodes(nodes), nodes_get_size(nodes));
        exit(1);
    }
    return result;
...
int
_zdd_try_make_node(ZDD *destination, uint32_t var, ZDD low, ZDD high)
{
    if (destination == NULL || var > UINT32_C(0x00ffffff) ||
        low == zdd_invalid || high == zdd_invalid) {
 ...
 */
int zdd_from_mtbdd_CALL(lace_worker* lace, ZDD *destination, MTBDD dd, MTBDD dom)
{
    if (destination == NULL || dd == mtbdd_invalid || dom == mtbdd_invalid) return SYLVAN_ERR_INVALID;

    /* Special treatment for False */
    if (dd == mtbdd_undefined) { *destination = zdd_false; return SYLVAN_OK; }
    if (dd == bdd_true && dom == bdd_true) { *destination = zdd_base; return SYLVAN_OK; }
    if (dom == bdd_true) {
    ...
    if (cache_get3(CACHE_ZDD_FROM_MTBDD, dd, dom, 0, &computed)) {
    ...
    if (dd == bdd_true || mtbddnode_isleaf(dd_node)) {
        ...
        status = zdd_from_mtbdd_CALL(lace, &computed, dd, dom_next);
        ...
        zdd_from_mtbdd_SPAWN(lace, &high, dd1, dom_next);
        status = zdd_from_mtbdd_CALL(lace, &low, dd0, dom_next);
        int high_status = zdd_from_mtbdd_SYNC(lace);
    ...
    /* Store in cache */
    if (cache_put3(CACHE_ZDD_FROM_MTBDD, dd, dom, 0, computed)) {
...
int
zdd_from_bdd_CALL(lace_worker* lace, ZDD *destination, BDD dd, BDDSET domain)
{
    if (destination == NULL || dd == mtbdd_invalid || domain == mtbdd_invalid) return SYLVAN_ERR_INVALID;
 ...
 */
int zdd_to_mtbdd_CALL(lace_worker* lace, MTBDD *destination, ZDD dd, BDDSET dom)
{
    if (destination == NULL || dd == zdd_invalid || dom == mtbdd_invalid) return SYLVAN_ERR_INVALID;

    /* Special treatment for True and False */
    if (dd == zdd_false) { *destination = mtbdd_undefined; return SYLVAN_OK; }
    if (dd == zdd_base && bdd_set_is_empty(dom)) { *destination = bdd_true; return SYLVAN_OK; }
    ...
    if (cache_get3(CACHE_ZDD_TO_MTBDD, dd, dom, 0, &computed)) {
    ...
    if (dd == zdd_base || zddnode_isleaf(dd_node)) {
        ...
        status = zdd_to_mtbdd_CALL(lace, &computed, dd, dom_next);
        if (status == SYLVAN_OK) status = _mtbdd_try_make_node(&computed, dom_var, computed, mtbdd_undefined);
    } else {
        /* Get variables */
        ...
        zdd_to_mtbdd_SPAWN(lace, &high, dd1, dom_next);
        status = zdd_to_mtbdd_CALL(lace, &low, dd0, dom_next);
        int high_status = zdd_to_mtbdd_SYNC(lace);
        if (status == SYLVAN_OK) status = high_status;
        if (status == SYLVAN_OK) status = _mtbdd_try_make_node(&computed, dom_var, low, high);
        mtbdd_refs_popptr(2);
    }
    ...
    /* Store in cache */
    if (cache_put3(CACHE_ZDD_TO_MTBDD, dd, dom, 0, computed)) {
...
int
bdd_from_zdd_CALL(lace_worker* lace, BDD *destination, ZDD dd, BDDSET domain)
{
    return zdd_to_mtbdd_CALL(lace, destination, dd, domain);
}

int
zdd_cofactor_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDD cube, BDDSET domain)
{
    if (destination == NULL || dd == zdd_invalid || cube == mtbdd_invalid || domain == mtbdd_invalid) {
        return SYLVAN_ERR_INVALID;
    }

    BDDSET cube_support = mtbdd_invalid;
    mtbdd_refs_pushptr(&cube_support);
    int status = mtbdd_support_CALL(lace, &cube_support, cube);
    if (status != SYLVAN_OK) {
        mtbdd_refs_popptr(1);
        return status;
    }
    ...
    BDD bdd = mtbdd_invalid;
    BDD cofactor = mtbdd_invalid;
    BDDSET result_domain = mtbdd_invalid;
    ZDD computed = zdd_invalid;
    mtbdd_refs_pushptr(&bdd);
    mtbdd_refs_pushptr(&cofactor);
    mtbdd_refs_pushptr(&result_domain);
    zdd_refs_pushptr(&computed);

    status = bdd_from_zdd_CALL(lace, &bdd, dd, domain);
    if (status == SYLVAN_OK) status = bdd_cofactor(&cofactor, bdd, cube);
    if (status == SYLVAN_OK) status = bdd_set_difference(&result_domain, domain, cube_support);
    if (status == SYLVAN_OK) status = zdd_from_bdd_CALL(lace, &computed, cofactor, result_domain);
...
int
zdd_extend_domain_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDDSET newvars, int value)
{
    if (destination == NULL || dd == zdd_invalid || newvars == mtbdd_invalid) return SYLVAN_ERR_INVALID;

    ZDD zdd_newvars = zdd_invalid;
    zdd_refs_pushptr(&zdd_newvars);
    int status = zdd_from_mtbdd_CALL(lace, &zdd_newvars, bdd_true, newvars);
    if (status == SYLVAN_OK) status = zdd_extend_domain_internal_CALL(lace, destination, dd, zdd_newvars, value);
    zdd_refs_popptr(1);
    return status;
}

int
zdd_lift_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDDSET old_domain, BDDSET new_domain)
{
    if (destination == NULL || dd == zdd_invalid ||
        old_domain == mtbdd_invalid || new_domain == mtbdd_invalid) {
        return SYLVAN_ERR_INVALID;
    }
    ...
    int status = zdd_support_CALL(lace, &support, dd);
    if (status != SYLVAN_OK) {
        mtbdd_refs_popptr(2);
        return status;
    }
    ...
    status = bdd_set_difference(&added, new_domain, old_domain);
    if (status == SYLVAN_OK) status = zdd_extend_domain_CALL(lace, destination, dd, added, 2);
    mtbdd_refs_popptr(2);
    return status;
}
 ...
 */
int zdd_extend_domain_internal_CALL(lace_worker* lace, ZDD *destination, ZDD set, ZDD newvars, int value)
{
    if (destination == NULL || set == zdd_invalid || newvars == zdd_invalid) return SYLVAN_ERR_INVALID;

    /**
     * Terminal cases
     */
    if (value == 0) { *destination = set; return SYLVAN_OK; }
    if (value != 1 && value != 2) return SYLVAN_ERR_INVALID;
    if (set == zdd_false) { *destination = zdd_false; return SYLVAN_OK; }
    if (newvars == zdd_base) { *destination = set; return SYLVAN_OK; }
    ...
    ZDD computed = zdd_invalid;
    zdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_EXTEND_DOMAIN, set, newvars, (uint64_t)(unsigned int)value, &computed)) {
    ...
    if (nv_var < set_var) {
        if (value == 1) {
            status = zdd_extend_domain_internal_CALL(lace, &computed, set, nv_next, value);
            if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, nv_var, zdd_false, computed);
        } else {
            status = zdd_extend_domain_internal_CALL(lace, &computed, set, nv_next, value);
            if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, nv_var, computed, computed);
        }
    } else {
        if (nv_var == set_var) {
            zdd_refs_popptr(1);
        ...
        zdd_refs_pushptr(&low);
        zdd_refs_pushptr(&high);
        zdd_extend_domain_internal_SPAWN(lace, &high, set1, newvars, value);
        status = zdd_extend_domain_internal_CALL(lace, &low, set0, newvars, value);
        int high_status = zdd_extend_domain_internal_SYNC(lace);
        if (status == SYLVAN_OK) status = high_status;
        if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, set_var, low, high);
        zdd_refs_popptr(2);
    }
     ...
     * Put in cache
     */
    if (cache_put3(CACHE_ZDD_EXTEND_DOMAIN, set, newvars, (uint64_t)(unsigned int)value, computed)) {
 ...
 */
int zdd_support_CALL(lace_worker* lace, BDDSET *destination, ZDD dd)
{
    if (destination == NULL || dd == zdd_invalid) return SYLVAN_ERR_INVALID;
    if (dd == zdd_base || dd == zdd_false) {
        *destination = bdd_set_empty();
        return SYLVAN_OK;
    }
    const zddnode* dd_node = ZDD_GETNODE(dd);
    if (zddnode_isleaf(dd_node)) {
        *destination = bdd_set_empty();
        return SYLVAN_OK;
    }
    ...
    mtbdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_SUPPORT, dd, 0, 0, &computed)) {
    ...
    mtbdd_refs_pushptr(&low);
    mtbdd_refs_pushptr(&high);
    zdd_support_SPAWN(lace, &low, dd0);
    int status = zdd_support_CALL(lace, &high, dd1);
    int low_status = zdd_support_SYNC(lace);
    if (status == SYLVAN_OK) status = low_status;
    if (status == SYLVAN_OK) status = bdd_set_union(&computed, low, high);
    if (status == SYLVAN_OK) status = bdd_set_add(&computed, computed, zddnode_getvariable(dd_node));
    mtbdd_refs_popptr(2);
    if (status != SYLVAN_OK) {
        mtbdd_refs_popptr(1);
        return status;
     ...
     * Put in cache
     */
    if (cache_put3(CACHE_ZDD_SUPPORT, dd, 0, 0, computed)) {
...
int zdd_and_CALL(lace_worker* lace, ZDD *destination, ZDD a, ZDD b)
{
    ...
    if (a == zdd_base) {
         ...
         * Get the vars
         */
        const zddnode* a_node = ZDD_GETNODE(a);
        const uint32_t a_var = zddnode_getvariable(a_node);
        const zddnode* b_node = ZDD_GETNODE(b);
 ...
 */
int
zdd_ite_CALL(lace_worker* lace, ZDD *destination, ZDD a, ZDD b, ZDD c, BDDSET domain)
{
    if (destination == NULL || a == zdd_invalid || b == zdd_invalid ||
        c == zdd_invalid || domain == mtbdd_invalid) {
        return SYLVAN_ERR_INVALID;
    }

    ZDD zdd_domain = zdd_invalid;
    zdd_refs_pushptr(&zdd_domain);
    int status = zdd_from_mtbdd_CALL(lace, &zdd_domain, bdd_true, domain);
    if (status == SYLVAN_OK) status = zdd_ite_internal_CALL(lace, destination, a, b, c, zdd_domain);
    zdd_refs_popptr(1);
    return status;
}

int zdd_ite_internal_CALL(lace_worker* lace, ZDD *destination, ZDD a, ZDD b, ZDD c, ZDD dom)
{
    if (destination == NULL || a == zdd_invalid || b == zdd_invalid ||
        c == zdd_invalid || dom == zdd_invalid) {
        return SYLVAN_ERR_INVALID;
    }
     ...
     * Trivial cases
     */
    if (a == zdd_false) { *destination = c; return SYLVAN_OK; }
    if (a == b) return zdd_or_CALL(lace, destination, a, c);
    if (a == c || c == zdd_false) return zdd_and_CALL(lace, destination, a, b);
    ...
    if (minvar == UINT32_MAX || dom == zdd_base || dom == zdd_false) return SYLVAN_ERR_INVALID;

    /**
     * Move dom to minvar
     */
    zddnode* dom_node = ZDD_GETNODE(dom);
    uint32_t dom_var = zddnode_getvariable(dom_node);
    ZDD dom_next = zddnode_high(dom, dom_node);
    while (dom_var != minvar) {
        if (dom_var > minvar || dom_next == zdd_base || dom_next == zdd_false) return SYLVAN_ERR_INVALID;
        dom = dom_next;
     ...
     *   - ITE(a,0,1) ==> not(a)
     */
    if (a == dom) { *destination = b; return SYLVAN_OK; }
    if (b == dom) return zdd_or_CALL(lace, destination, a, c);
    if (b == zdd_false && c == dom) return zdd_not_internal_CALL(lace, destination, a, dom);
    ...
    ZDD computed = zdd_invalid;
    zdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_ITE, a, b, c, &computed)) {
    ...
    zdd_refs_pushptr(&low);
    zdd_refs_pushptr(&high);
    zdd_ite_internal_SPAWN(lace, &low, a0, b0, c0, dom_next);
    int status = zdd_ite_internal_CALL(lace, &high, a1, b1, c1, dom_next);
    int low_status = zdd_ite_internal_SYNC(lace);
    if (status == SYLVAN_OK) status = low_status;
     ...
     * Cache the result
     */
    if (cache_put3(CACHE_ZDD_ITE, a, b, c, computed)) {
 ...
 */
int
zdd_not_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDDSET domain)
{
    if (destination == NULL || dd == zdd_invalid || domain == mtbdd_invalid) return SYLVAN_ERR_INVALID;

    ZDD zdd_domain = zdd_invalid;
    zdd_refs_pushptr(&zdd_domain);
    int status = zdd_from_mtbdd_CALL(lace, &zdd_domain, bdd_true, domain);
    if (status == SYLVAN_OK) status = zdd_not_internal_CALL(lace, destination, dd, zdd_domain);
    zdd_refs_popptr(1);
    return status;
}

int zdd_not_internal_CALL(lace_worker* lace, ZDD *destination, ZDD dd, ZDD dom)
{
    if (destination == NULL || dd == zdd_invalid || dom == zdd_invalid) return SYLVAN_ERR_INVALID;

    /**
     * Trivial cases (abusing the notion of dom representing True for all assignments)
     */
    if (dd == dom) { *destination = zdd_false; return SYLVAN_OK; }
    if (dd == zdd_false) { *destination = dom; return SYLVAN_OK; }
    if (dom == zdd_base || dom == zdd_false) return SYLVAN_ERR_INVALID;
    ...
    ZDD computed = zdd_invalid;
    zdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_NOT, dd, dom, 0, &computed)) {
    ...
    if (dom_var < dd_var) {
        ...
        zdd_refs_pushptr(&low);
        status = zdd_not_internal_CALL(lace, &low, dd, dom_next);
        if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, dom_var, low, high);
        zdd_refs_popptr(1);
    } else {
        if (dd_node == NULL) {
        ...
        zdd_refs_pushptr(&low);
        zdd_refs_pushptr(&high);
        zdd_not_internal_SPAWN(lace, &low, dd0, dom_next);
        status = zdd_not_internal_CALL(lace, &high, dd1, dom_next);
        int low_status = zdd_not_internal_SYNC(lace);
        if (status == SYLVAN_OK) status = low_status;
     ...
     * Cache the result
     */
    if (cache_put3(CACHE_ZDD_NOT, dd, dom, 0, computed)) {
 ...
 */
int
zdd_exists_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDDSET variables)
{
    if (destination == NULL || dd == zdd_invalid || variables == mtbdd_invalid) return SYLVAN_ERR_INVALID;
    ...
    if (status == SYLVAN_OK) status = zdd_exists_internal_CALL(lace, destination, dd, zdd_variables);
    zdd_refs_popptr(1);
    return status;
}

int zdd_exists_internal_CALL(lace_worker* lace, ZDD *destination, ZDD dd, ZDD vars)
{
    if (destination == NULL || dd == zdd_invalid || vars == zdd_invalid) return SYLVAN_ERR_INVALID;

    /**
     * Trivial cases
     */
    if (dd == zdd_base) { *destination = vars; return SYLVAN_OK; }
    if (dd == zdd_false) { *destination = dd; return SYLVAN_OK; }
    if (vars == zdd_base) { *destination = dd; return SYLVAN_OK; }
    ...
    ZDD computed = zdd_invalid;
    zdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_EXISTS, dd, vars, 0, &computed)) {
    ...
    const zddnode* dd_node = ZDD_GETNODE(dd);
    const uint32_t dd_var = zddnode_getvariable(dd_node);
    const zddnode* vars_node = ZDD_GETNODE(vars);
    ...
    int status = SYLVAN_OK;
    if (vars_var < dd_var) {
        status = zdd_exists_internal_CALL(lace, &computed, dd, zddnode_high(vars, vars_node));
        if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, vars_var, computed, computed);
    } else {
        /**
         * Get cofactors
         */
        ...
        if (vars_var == dd_var) {
            ...
            if (dd0 == dd1) {
                status = zdd_exists_internal_CALL(lace, &computed, dd0, vars_next);
            } else {
                ZDD low = zdd_invalid;
                ZDD high = zdd_invalid;
                zdd_refs_pushptr(&low);
                zdd_refs_pushptr(&high);
                zdd_exists_internal_SPAWN(lace, &low, dd0, vars_next);
                status = zdd_exists_internal_CALL(lace, &high, dd1, vars_next);
                int low_status = zdd_exists_internal_SYNC(lace);
                if (status == SYLVAN_OK) status = low_status;
                if (status == SYLVAN_OK) status = zdd_or_CALL(lace, &computed, low, high);
                zdd_refs_popptr(2);
            }
            ...
            zdd_refs_pushptr(&high);
            if (dd0 == dd1) {
                status = zdd_exists_internal_CALL(lace, &low, dd0, vars);
                high = low;
            } else {
                zdd_exists_internal_SPAWN(lace, &low, dd0, vars);
                status = zdd_exists_internal_CALL(lace, &high, dd1, vars);
                int low_status = zdd_exists_internal_SYNC(lace);
                if (status == SYLVAN_OK) status = low_status;
            }
             ...
             * Compute result node
             */
            if (status == SYLVAN_OK) status = _zdd_try_make_node(&computed, dd_var, low, high);
            zdd_refs_popptr(2);
        }
    }
     ...
     * Cache the result
     */
    if (cache_put3(CACHE_ZDD_EXISTS, dd, vars, 0, computed)) {
 ...
 */
int
zdd_project_CALL(lace_worker* lace, ZDD *destination, ZDD dd, BDDSET domain)
{
    if (destination == NULL || dd == zdd_invalid || domain == mtbdd_invalid) return SYLVAN_ERR_INVALID;
    ...
    zdd_refs_pushptr(&zdd_domain);
    int status = zdd_from_mtbdd_CALL(lace, &zdd_domain, bdd_true, domain);
    if (status == SYLVAN_OK) status = zdd_project_internal_CALL(lace, destination, dd, zdd_domain);
    zdd_refs_popptr(1);
    return status;
}

int zdd_project_internal_CALL(lace_worker* lace, ZDD *destination, ZDD dd, ZDD dom)
{
    if (destination == NULL || dd == zdd_invalid || dom == zdd_invalid) return SYLVAN_ERR_INVALID;
     ...
     * Trivial cases
     */
    if (dd == zdd_base) { *destination = dd; return SYLVAN_OK; }
    if (dd == zdd_false) { *destination = dd; return SYLVAN_OK; }
    if (dom == zdd_base) { *destination = zdd_base; return SYLVAN_OK; } // assuming dd is indeed Boolean
    ...
    ZDD computed = zdd_invalid;
    zdd_refs_pushptr(&computed);
    if (cache_get3(CACHE_ZDD_PROJECT, dd, dom, 0, &computed)) {
    ...
    if (dd_var < dom_var) {
         ...
         */
        if (dd0 == dd1) {
            status = zdd_project_internal_CALL(lace, &computed, dd0, dom);
        } else {
            ZDD low = zdd_invalid;
            ZDD high = zdd_invalid;
            zdd_refs_pushptr(&low);
            zdd_refs_pushptr(&high);
            zdd_project_internal_SPAWN(lace, &low, dd0, dom);
            status = zdd_project_internal_CALL(lace, &high, dd1, dom);
            int low_status = zdd_project_internal_SYNC(lace);
            if (status == SYLVAN_OK) status = low_status;
            if (status == SYLVAN_OK) status = zdd_or_CALL(lace, &computed, low, high);
            zdd_refs_popptr(2);
        }
        ...
        zdd_refs_pushptr(&high);
        if (dd0 == dd1) {
            status = zdd_project_internal_CALL(lace, &low, dd0, dom_next);
            high = low;
        } else {
            zdd_project_internal_SPAWN(lace, &low, dd0, dom_next);
            status = zdd_project_internal_CALL(lace, &high, dd1, dom_next);
            int low_status = zdd_project_internal_SYNC(lace);
            if (status == SYLVAN_OK) status = low_status;
        }
     ...
     * Cache the result
     */
    if (cache_put3(CACHE_ZDD_PROJECT, dd, dom, 0, computed)) {
...
{
    zddnode* n = ZDD_GETNODE(zdd); // also works for zdd_false
    if (zddnode_getmark(n)) return;
    zddnode_setmark(n, 1);
...
ZDD zdd_isop_CALL(lace_worker* lace, MTBDD L, MTBDD U, MTBDD* bddresptr)
{
     ...
     */
    const mtbddnode* L_node = MTBDD_GETNODE(L);
    const mtbddnode* U_node = MTBDD_GETNODE(U);
